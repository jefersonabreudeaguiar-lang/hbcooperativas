/**
 * Zera dados de teste dos cooperados na nuvem:
 * - entregas/notas e fotos (hb-entregas + notas_pedido)
 * - lançamentos operacionais (operacional.json)
...
const OPERATIONAL_RESET_VERSION = 7;
...
for (const cnpj of cnpjs) {
  ...
  totalUsers += users;
  console.log(
    `CNPJ ${cnpj}: ${notas} entrega(s), operacional zerado, ${cooperados} cadastro(s) cooperado, ${users} conta(s) cooperado.`
  );
}

console.log(
  `Limpeza concluída — ${totalNotas} entrega(s), ${totalCooperados} cadastro(s), ${totalUsers} conta(s) cooperado removidos.`
);

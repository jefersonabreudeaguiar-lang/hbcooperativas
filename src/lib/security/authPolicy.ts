...
/** Perfis permitidos no cadastro público (/api/auth/register). Equipe usa /api/equipe/provision. */
export const PUBLIC_REGISTER_ROLES: UserRole[] = ["cooperado", "parceiro"];

/** Perfis permitidos na primeira sincronização na nuvem (/api/auth/provision). */
export const PROVISION_NEW_USER_ROLES: UserRole[] = ["cooperado", "parceiro"];

...
function isMissingTable(error: { code?: string; message?: string } | null): boolean {
  ...
  return /password_reset_tokens/i.test(msg) && /(could not find|does not exist|schema cache)/i.test(msg);
}

export async function isPasswordResetTableReady(supabase: SupabaseClient): Promise<boolean> {
  const { error } = await supabase.from("password_reset_tokens").select("id").limit(1);
  return !error;
}
...
export async function applyPasswordResetSchemaSql(): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  const sql = readFileSync(
    resolve(process.cwd(), "supabase/migrations/APPLY_PASSWORD_RESET.sql"),
    "utf8"
  );
...
export { isMissingTable as isPasswordResetTableMissing };

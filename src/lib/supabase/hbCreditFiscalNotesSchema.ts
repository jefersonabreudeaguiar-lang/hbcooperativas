...
const FISCAL_NOTES_MIGRATION_SQL = readFileSync(
  resolve(process.cwd(), "supabase/migrations/20260829400000_hb_credit_fiscal_notes.sql"),
  "utf8"
);
...
export async function checkHbCreditFiscalNotesSchema(supabase: SupabaseClient): Promise<{
  fiscalNotesTableOk: boolean;
  ok: boolean;
}> {
  const probe = await supabase.from("hb_credit_fiscal_notes").select("id").limit(1);
  const fiscalNotesTableOk =
    !probe.error || !/hb_credit_fiscal_notes/i.test(probe.error.message ?? "");
  return { fiscalNotesTableOk, ok: fiscalNotesTableOk };
}

export async function applyHbCreditFiscalNotesSchemaSql(): Promise<
...
> {
  ...
  for (const conn of buildConnectionCandidates(projectRef, dbPassword)) {
    ...
    if (!client) continue;
    try {
      await client.query(FISCAL_NOTES_MIGRATION_SQL);
      await client.end();
      return { ok: true, applied: "20260829400000_hb_credit_fiscal_notes.sql" };
    } catch (e) {

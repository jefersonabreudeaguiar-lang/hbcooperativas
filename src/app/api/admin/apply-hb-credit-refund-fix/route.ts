...
import {
  applyHbCreditRefundFixSchemaSql,
  checkHbCreditRefundFixSchema,
} from "@/lib/supabase/hbCreditRefundFixSchema";
...
export async function POST(request: Request) {
  ...
  const before = await checkHbCreditRefundFixSchema(supabase);
  if (before.ok) {
    return NextResponse.json({ ok: true, alreadyApplied: true, schema: before });
  }

  const applied = await applyHbCreditRefundFixSchemaSql();
  if (!applied.ok) {
  ...
  const after = await checkHbCreditRefundFixSchema(supabase);
  return NextResponse.json({
    ok: after.ok,
...
export async function GET(request: Request) {
  ...
  const schema = await checkHbCreditRefundFixSchema(getSupabaseAdmin()!);
